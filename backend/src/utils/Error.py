...
class Error:
    ...
    def getDot(self, id) -> str:
        if self.type == TypeError.LEXICAL:
            return f'<tr><td bgcolor="white">{id}</td><td bgcolor="white">{self.line}</td><td bgcolor="white">{self.column}</td><td bgcolor="white">{self.type}</td><td bgcolor="white">{self.description}</td></tr>'
